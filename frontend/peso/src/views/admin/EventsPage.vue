<template>
  <div class="page">
    ...
    <div v-if="view === 'list'" class="events-list">
      ...
      <div v-for="event in filteredEvents" :key="event.id"
        ...
        <div class="event-actions">
          ...
          <!-- Edit: locked for Ongoing / Completed -->
          <button
            class="act-btn edit"
          ...
          <!-- Archive: locked for Ongoing / Completed -->
          <button
    ...
    <!-- CALENDAR VIEW — shows ALL events including hidden-from-list completed ones -->
    ...
    <!-- LOCK TOOLTIP MODAL (when clicking locked buttons) -->
    <transition name="toast">
...
export default {
  ...
  data() {
    ...
    return {
      ...
      events: [],       // all events from API (used for calendar)
      currentPage: 1,
      lastPage: 1,
      totalEvents: 0,
    }
  ...
  computed: {
    // ── Visibility rules ────────────────────────────────────────────
    // A completed event disappears from the LIST 1 day after its event date.
    // It remains visible in the calendar always.
    isHiddenFromList() {
      return (event) => {
    ...
    // Locked = cannot edit or archive (Ongoing or Completed regardless of hide)
    isLocked() {
    ...
    filteredEvents() {
      return this.events.filter(e => {
        // Hide completed events older than 1 day from the list view
    ...
    // Calendar uses ALL events (no hiding)
    calCells() {
  ...
  methods: {
    ...
    async fetchEvents() {
      try {
        ...
        // Don't filter by status on API — we need all statuses for calendar hide logic
        // Only add status if user explicitly filtered via dropdown (not tab)
    ...
    updateTabCounts() {
      // Tabs reflect what's visible in the list (hidden-from-list events excluded from "All" count)
      const visible = this.events.filter(e => !this.isHiddenFromList(e))
      this.statusTabs[0].count = visible.length
      this.statusTabs[1].count = visible.filter(e => e.status === 'Upcoming').length
      this.statusTabs[2].count = visible.filter(e => e.status === 'Ongoing').length
      this.statusTabs[3].count = visible.filter(e => e.status === 'Completed').length
    },
    ...
    showLockMessage(event) {
      ...
      this.lockToast = {
        show: true,
        text: msg,
        _timer: setTimeout(() => { this.lockToast.show = false }, 3000),
      }
    },
...
/* Locked card styling */
.event-card--locked { background: #fafafa; border-color: #f1f5f9; }
.event-card--locked .event-title { color: #64748b; }
.event-card--locked .event-desc  { color: #cbd5e1; }

/* Locked ribbon at top of card */
.locked-ribbon {
...
/* Push card content down when ribbon is showing */
.event-card--locked { padding-top: 36px; }
