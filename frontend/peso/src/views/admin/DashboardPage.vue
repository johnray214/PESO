  ...
  <div class="dashboard-page">
    ...
    <div class="mid-row">
      <div class="card chart-card">
        ...
        <div class="svg-chart-wrap" ref="regWrap">
          <!-- Tooltip rendered in wrapper div so position is relative to it -->
          <transition name="tip">
          ...
          <svg ref="regSvg"
            ...
            <!-- Dots — purely visual, no mouse events needed -->
            ...
            <!-- Wide invisible hit bands over each data column for easy hover -->
    ...
    <div class="mid-row">
      <div class="card chart-card">
        ...
        <div class="svg-chart-wrap" ref="pmWrap">
          <transition name="tip">
          ...
          <svg ref="pmSvg"
            ...
            <!-- Dots — no mouse events, SVG mousemove handles everything -->
    ...
    <div class="skills-gap-row">
      <div class="card">
        <div class="card-header">
          <div><h3>Indem Skills</h3><p class="card-sub">Most in-demand skills from active job postings this month</p></div>
          <span class="live-badge">Live</span>
        </div>
...
export default {
  ...
  data() {
    return {
      ...
      // tooltip state — single plain object or null
      regTip: null,   // { i, label, jobseekers, employers, x, y }
      pmTip:  null,   // { i, label, placement, processing, registration, rejection, x, y }
    }
  },
  ...
  async mounted() {
    try {
      ...
      if (d.trendingSkills?.length) {
        ...
        this.trendingSkills = d.trendingSkills.map((s, i) => ({
          ...(this.trendingSkills[i] ?? {}),
          name: s.name, count: s.count,
          color: colors[i%colors.length],
          change: this.trendingSkills[i]?.change ?? '—',
          up:     this.trendingSkills[i]?.up     ?? true,
        }))
      }
    ...
    this.$nextTick(() => {
      setTimeout(() => { this.regAnimated    = true }, 100)
      setTimeout(() => { this.pmAnimated     = true }, 220)
      setTimeout(() => { this.donutAnimated  = true }, 160)
      setTimeout(() => { this.skillsAnimated = true }, 320)
      setTimeout(() => { this.gapsAnimated   = true }, 380)
    })
  },
  ...
  methods: {
    ...
    // ── Core tooltip engine ────────────────────────────────────────────────
    // Uses SVGElement.getScreenCTM() to accurately convert mouse coords into
    // SVG viewBox space — works correctly at any CSS scale.
    ...
    _closestIdx(svgX, getXFn, count) {
      ...
      // snap radius = half the column width, so entire column is a hot zone
      const colW = 560/Math.max(count-1,1)
      return bestDist <= colW/2+4 ? best : null
    },

    // Position tooltip bubble relative to the wrapper div.
    ...
    onRegMove(evt) {
      ...
      // Spread raw into a plain object — avoids Proxy stale-read issue
      this.regTip = {
        i:          idx,
        label:      String(raw.label      ?? ''),
        jobseekers: Number(raw.jobseekers ?? 0),
        employers:  Number(raw.employers  ?? 0),
        x: pos.x,
        y: pos.y,
      }
    },
    ...
    onPmMove(evt) {
      ...
      this.pmTip = {
        i:            idx,
        label:        String(raw.label        ?? ''),
        placement:    Number(raw.placement    ?? 0),
        processing:   Number(raw.processing   ?? 0),
        registration: Number(raw.registration ?? 0),
        rejection:    Number(raw.rejection    ?? 0),
        x: pos.x,
        y: pos.y,
      }
    },
...
.tooltip-label {
  font-weight: 700; font-size: 10px; color: #94a3b8;
